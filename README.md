...
| **Vacaciones (base)**         | Administración de `PROG_VAC` (períodos con `DIAS`/`DISP`, CRUD y carga Excel `N_IDE, N_CONT, PER_INI, PER_FIN, DIAS, DISP, EST` con fecha de corte, vista previa y ajustes versionados con motivo) y de festivos (borradores versionados y publicación). El empleado puede calcular fechas de disfrute y de retorno con días hábiles y festivos (`/me/vacations/preview`); la solicitud y su aprobación siguen pendientes.                |
