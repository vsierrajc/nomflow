...
| 4 | ESS-LEAVE-002/003, ESS-HOL-001 (**ESS-LEAVE-001 hecho**: `PROG_VAC` con CRUD y carga Excel; festivos con CRUD y publicación; cálculo de fechas y vista previa listos. Faltan: solicitud, revisiones, aprobación del jefe y final, `VACACIONES`, PDF con firmas, festivos por API y por Excel) | `PROG_VAC`, solicitudes multi-período, días hábiles con festivos, aprobación del jefe y final, PDF con firmas (6.2) | Empezar por `HolidayCalendar` (API de festivos con clave, respaldo local) y `PROG_VAC` (importación + CRUD). `resolveAreaManager` ya existe |
| 5 | ESS-PERM-001 | Permisos por tipo, sin consumir `DISP` (6.2) | Depende de las solicitudes y del catálogo de tipos |
